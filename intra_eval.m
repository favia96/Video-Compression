...
hist_intra=zeros(length(steps),length(frames),8,8,9,11,4);
...
bitrates=zeros(length(steps),length(frames),8,8);
for i=1:length(steps)
    for j=1:length(frames)
        for blcok_index1=1:8
             for block_index2=1:8
                 prob=reshape(hist_intra(i,j,blcok_index1,block_index2,:,:,:),1,9*11*4);
