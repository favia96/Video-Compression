function [recon_frames,psnr_cond_repl,bitrates_cond_repl,J_intra_mode,J_copy_mode] = cond_repl_coding(frames,frame_size,video_height,video_width,steps,block_size,fps)
    ...
    for i = 1 : length(steps)
        for j = 1 : length(frames)
%            mse_intra(i,j) = my_mse(recon_frames{i,j},frames{j}); %psnr
    ...
    for i = 1 : length(steps)
        for j = 1 : length(frames)
            for index1 = 1 :  video_height/block_size % 1 : 9
                for index2 = 1 : video_width/block_size % 1 : 11
                    J_intra_mode(i,j,index1,index2) = mse_intra_mode(i,j,index1,index2) + 0.002*steps(i).^2 * (sum(sum(bitrates(i,j,:,:))) + 1);

                    if j >= 2 % after second frame
                        J_copy_mode(i,j,index1,index2) = mse_copy_mode(i,j,index1,index2) + 0.002*steps(i).^2 * 1; %(sum(sum(bitrates(i,j-1,:,:))) + 1);
                    else
                        J_copy_mode(i,j,index1,index2) = J_intra_mode(i,j,index1,index2);
                    end
                    
                    if J_intra_mode(i,j,index1,index2) > J_copy_mode(i,j,index1,index2)
                        enc_dct16_quan{i,j}{index1,index2} = enc_dct16_quan{i,j-1}{index1,index2}; % copy from previous frame
                        bit(i) = bit(i) + 1;
                    else
                        bit(i) = bit(i) + sum(sum(bitrates(i,j,:,:))) + 1;
                    end                             
                end
            end 
        end
    ...
    for i = 1 : length(steps)
        for j = 1 : length(frames)
            for m = 1 : video_height/block_size
                for n = 1 : video_width/block_size
                   dec_idct16{i,j}{m,n} = blkproc(enc_dct16_quan{i,j}{m,n},[block_size/2 block_size/2],@idct2);
                end
            end
            recon_frames{i,j} = cell2mat(dec_idct16{i,j});
    ...
    [psnr_cond_repl, ~] = intra_eval(frames,video_height,video_width,recon_frames,enc_dct16_quan,steps,block_size,fps,1);
    bitrates_cond_repl = bit / 50 * 30 / 1000;
